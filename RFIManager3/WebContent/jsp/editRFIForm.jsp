...
function changeBoqNumber()
{
	...
	var activity = document.getElementById("activity").value;
		...
		{
			url = "/RFIManager3/getBoqWorkItems.htm?boqNumber="+encodeURIComponent(boqNumber)+"&activity="+encodeURIComponent(activity);
		...
		{
			url = "/RFIManager3/getBoqWorkItems.htm?noItemDesc=1"+"&activity="+encodeURIComponent(activity);
			jQuery.ajaxSetup({async:false});
			boqURL(url);
			jQuery.ajaxSetup({async:true});
		}
...
function changeWorkItem()
{
	...
	{
		url = "/RFIManager3/getBoqWorkItems.htm?itemDesc="+encodeURIComponent(itemDescription)+"&activity="+encodeURIComponent(activity);
		jQuery.ajaxSetup({async:false});
		boqURL(url);
		jQuery.ajaxSetup({async:true});
	...
	{
		url = "/RFIManager3/getBoqWorkItems.htm?noBoqNumber=1"+"&activity="+encodeURIComponent(activity);
		jQuery.ajaxSetup({async:false});
		itemURL(url);
		jQuery.ajaxSetup({async:true});
	}
...
mygrid.load("http://localhost:8080/RFIManager3/rfiXml.htm");
