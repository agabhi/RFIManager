...
public class TodaysRfiXmlController extends AbstractController
{
	...
	static
	{
		...
		columnTypeMap.put("bill_number","convertNumeric");
			 										...
			 										HttpServletResponse response) throws Exception
	{
		 ...
		 else
		 {
			...
			where.append(" WHERE DATE_FORMAT(issue_date,'%Y-%m-%d') = '"+dateString+"' ");
	 ...
	 public void write(WritableWorkbook workbook, List<ExpandedRFI>expandedRFIList) throws IOException, WriteException {
		...
		private void createLabel(WritableSheet sheet)
			...
			// Write a few headers
			addLabel(sheet, 0, 0, "Issue Date");
			addLabel(sheet, 1, 0, "RFI Number");
			addLabel(sheet, 2, 0, "BOQ No.");
			addLabel(sheet, 3, 0, "From Chainage");
			addLabel(sheet, 4, 0, "To Chainage");
			addLabel(sheet, 5, 0, "Side");
			addLabel(sheet, 6, 0, "Item");
			addLabel(sheet, 7, 0, "WI Remarks");
			addLabel(sheet, 8, 0, "Layer");
			addLabel(sheet, 9, 0, "Inspection date");
			addLabel(sheet, 10, 0, "Remarks");
		...
		private void createContent(WritableSheet sheet, List<ExpandedRFI> expandedRFIList) throws WriteException,
			...
			while(it.hasNext())
			{
				...
				addDate(sheet,0,rowNumber,expandedRFI.getIssueDate());
				addString(sheet,1,rowNumber,expandedRFI.getRfiCode()+expandedRFI.getRfiNumber());
				addString(sheet,2,rowNumber,expandedRFI.getBoqNumber());
				addNumber(sheet,3,rowNumber,expandedRFI.getFromChainage());
				addNumber(sheet,4,rowNumber,expandedRFI.getToChainage());
				addString(sheet,5,rowNumber,expandedRFI.getSide());
				addString(sheet,6,rowNumber,expandedRFI.getItemDescription());
				addString(sheet,7,rowNumber,expandedRFI.getWiRemarks());
				addString(sheet,8,rowNumber,expandedRFI.getLayer());
				addLongDate(sheet,9,rowNumber,expandedRFI.getInspectionDate());
				addString(sheet,10,rowNumber,expandedRFI.getRemarks());
