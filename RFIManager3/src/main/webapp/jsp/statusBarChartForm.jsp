...
<script src="<%=request.getContextPath()%>/js/jquery-1.5.2.min.js" type="text/javascript"></script>
...
function changeActivity()
{
	...
	url = "<%=request.getContextPath()%>/getBoqWorkItems.htm?noBoqNumber=1"+"&activity="+activity;
...
function saveBarChart()
{
	...
	if(isEmpty(document.getElementById("items").value))
	{
	...
	{
		alert("Please select one or more sides!");
		return false;
	}
	
	if(isEmpty(document.getElementById("barChartName").value))
	{
		alert("Please enter a barchart name!");
		return false;
	}
	
	var queryString = $('#drawStatusBarChartform').serialize();
	var url1 ="<%=request.getContextPath()%>/drawStatusbarChart.htm?"+queryString;
	$.ajaxSetup({async:false});	
	$.post(url1,
			  {saveBarChartName: document.getElementById("barChartName").value},
			  function(message){
				  //alert("Inside");
				  //alert(barChartHtml);
				  alert(message);
...
<table class="curvedEdges" style="width:100%;" cellspacing="0" cellpadding="5"  bgcolor="#F2F2F2" id="rfiDetailsTable"> 
				<tr>
			     ...
			     <tr>
			     	<td style="border:0">
		        		<label><font size="4">Select Status</font></label>
		        	</td>
		        	<td style="border:0">
						<form:select id="statuses" path="statuses" multiple="multiple" size="2"> 
			     ...
			     <tr>
			     	<td style="border:0">
		        		<label><font size="4">Select Activity</font></label>
		        	</td>
		        	<td style="border:0">
		       		...
		       		<td style="border:0">
		        		<label><font size="4">Select Items</font></label>
		        	</td>
		        	<td style="border:0">
						<form:select id="items" path="items" multiple="multiple" size="2"> 
							<form:options items="${workItemDescriptionList}" /> 
						</form:select> 
					</td>
					<td style="border:0">
		        		<label><font size="4">Select Sides</font></label>
		        	</td>
					<td style="border:0">
						<form:select id="sides" path="sides" multiple="multiple" size="2"> 
				...
				<tr>
					<td>
						<input id="saveButton" type="submit" value="Save Barchart" onclick="javascript: saveBarChart(); return false;">
						<form:input id="barChartName" path="barChartName" />
						<br>
