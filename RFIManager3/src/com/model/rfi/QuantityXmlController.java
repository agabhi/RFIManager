...
public class QuantityXmlController extends AbstractController
{
			 										...
			 										HttpServletResponse response) throws Exception
	{
		 ...
		 String deleteQuantity  =  request.getParameter("deleteQuantity");
			if(deleteQuantity != null && !StringUtils.isEmptyOrWhitespaceOnly(deleteQuantity))
			{
				String[] quantityId = request.getParameterValues("quantityId");
				if( quantityId != null && quantityId.length > 0)
				{
					for (int i = 0; i<quantityId.length; ++i)
					{
						rfiService.deleteQuantity(Long.valueOf(quantityId[i]));
					}
				}
			...
			if("true".equals(searchOn))
			{
			 ...
			 Integer quantityListSize = rfiService.getQuantityListSize(where.toString());
			 
			 if(quantityListSize > 0 && limit > 0)
			 {
				totalPages = (quantityListSize%limit)>0?quantityListSize/limit+1:quantityListSize/limit;
			 }
			...
			JAXBContext context = JAXBContext.newInstance(QuantityList.class);
			Marshaller m = context.createMarshaller();
			m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
			
			StringWriter sw = new StringWriter();
			List<Quantity> quantityList = rfiService.getQuantityList(start,limit, sidx, where.toString());
			
			m.marshal(new QuantityList(quantityList), sw); 
			String xml = sw.toString();
			sw.close();
			
			String rows = ("<rows><page>"+page+"</page><total>"+totalPages+"</total><records>"+quantityListSize+"</records>");
